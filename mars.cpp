...
void mars_encrypt(const WORD *in, WORD *out, const WORD *key) {
    ...
    for (i = 0; i < 16; i++) {
        ...
        if (i < 8) {
            /* first eight rounds in forward mode */
            out[dst1] += L;
            out[dst3] ^= R;
        } else {
            /* last eight rounds in backwards mode */
            out[dst3] += L;
            out[dst1] ^= R;
        }
        IVT_DEBUG(out[0], out[1], out[2], out[3]);
...
int makeKey(keyInstance *key, const BYTE direction, int keyLen, const char *keyMaterial) {
    ...
    for (i = 0, j = 0; i < keyLen / 4; i += 2, j++)
        reinterpret_cast<BYTE *>(tmpkey)[j] = static_cast<BYTE>((hex[static_cast<unsigned char>(keyMaterial[i])] << 4) |
                                                                hex[static_cast<unsigned char>(keyMaterial[i + 1])]);
...
int cipherInit(cipherInstance *cipher, const BYTE mode, const char *IV) {
    ...
    if ((mode == MODE_CBC) || (mode == MODE_CFB1)) {
        ...
        for (i = 0, j = 0; j < MAX_IV_SIZE; i += 2, j++)
            cipher->IV[j] = static_cast<BYTE>((hex[static_cast<unsigned char>(IV[i])] << 4) | hex[static_cast<unsigned
                                                  char>(IV[i + 1])]);
...
int blockEncrypt(cipherInstance *cipher, keyInstance *key, BYTE *input,
    ...
    if (cipher->mode == MODE_ECB) {
        ...
        cipher->CIV[3] = (cipher->CIV[3] << 1) | static_cast<WORD>(outBuffer[0]);
    } else
        return (BAD_CIPHER_MODE);
...
int blockDecrypt(cipherInstance *cipher, const keyInstance *key, BYTE *input,
    ...
    if (cipher->mode == MODE_ECB) {
        ...
        cipher->CIV[1] = (cipher->CIV[1] << 1) | (cipher->CIV[2] & 0x80000000);
        cipher->CIV[2] = (cipher->CIV[2] << 1) | (cipher->CIV[3] & 0x80000000);
        cipher->CIV[3] = (cipher->CIV[3] << 1) | static_cast<WORD>(input[0] & 1);
    } else
        return (BAD_CIPHER_MODE);
